...
export async function runMorning(): Promise<void> {
  ...
  const actionable = scored.filter(
    (j) => j.score >= SCORE_THRESHOLD && j.recommendation !== "skip"
  );
  ...
  if (actionable.length === 0) {
    ...
    for (const job of actionable) {
      ...
      spinner.succeed(
        `${chalk.bold(job.company)} — ${job.title} ` +
...
function printScoreSummary(jobs: ScoredJob[]): void {
  console.log(chalk.bold("\n📊  Job Scores:\n"));
  for (const job of jobs.slice(0, 20)) {
    const bar = scoreBar(job.score);
    const rec = recommendationLabel(job.recommendation);
    console.log(
      `  ${bar} ${chalk.bold(job.score.toString().padStart(3))}  ${rec}  ` +
...
function recommendationLabel(rec: ScoredJob["recommendation"]): string {
  if (rec === "apply") return chalk.green("[APPLY  ]");
  if (rec === "stretch") return chalk.yellow("[STRETCH]");
