 ...
 *
 *  buildDailyDigest()  — sent every morning after the run
 *  buildWeeklyReport() — sent on Fridays, summarising the whole week
 */
...
export function buildDailyDigest(
  ...
  const jobCards = actionable.length === 0
    ...
    : actionable
        .map((j) => {
          const scoreClass =
            j.score >= 70 ? "score-high" : j.score >= 50 ? "score-mid" : "score-low";
...
export function buildWeeklyReport(
  allApplied: AppliedJob[],
  weekStart: string,
  weekEnd: string
): string {
  // Jobs applied this week
  const thisWeek = allApplied.filter(
    (j) => j.appliedDate >= weekStart && j.appliedDate <= weekEnd
  );

  // Total pipeline stats across all time
  const counts: Record<AppliedJob["status"], number> = {
    applied: 0, screening: 0, interview: 0, offer: 0, rejected: 0, ghosted: 0,
  };
  for (const j of allApplied) counts[j.status]++;

  const responseRate =
    allApplied.length > 0
      ? Math.round(
          ((counts.screening + counts.interview + counts.offer + counts.rejected) /
            allApplied.length) *
            100
        )
      : 0;
  ...
  // ── Pipeline breakdown ──
  const statusLabels: { key: AppliedJob["status"]; label: string }[] = [
