...
async function scrapeGlassdoor(query, location) {
    const encodedQuery = encodeURIComponent(query);
    const encodedLocation = encodeURIComponent(location);
...
async function scrapeCWJobs(query, location) {
    const encodedQuery = encodeURIComponent(query);
    const encodedLocation = encodeURIComponent(location);
 ...
 */
export async function crawlJobBoards(query, location) {
 ...
 */
export async function crawlAndEnrichJobs(query, location, maxDescriptions = 10) {
    const jobs = await crawlJobBoards(query, location);
    if (jobs.length === 0) {
        return [];
    }
    // Fetch descriptions for top N jobs to save time
    const jobsWithDescriptions = await Promise.all(jobs.slice(0, maxDescriptions).map(fetchJobDescription));
