...
function buildTailorPrompt(profile, job) {
    const allBullets = profile.experience.flatMap((e) => e.bullets.map((b) => `[${e.role} @ ${e.company}] ${b}`));
...
export async function tailorCV(profile, job) {
